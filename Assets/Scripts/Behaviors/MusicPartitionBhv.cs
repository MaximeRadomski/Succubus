...
public class MusicPartitionBhv : MonoBehaviour
{
    ...
    public void Init(Realm opponentRealm, int nbPieces, Instantiator instantiator)
    {
        ...
        {
            var y = _yMaxNoteOffset - (Random.Range(0.0f, 12.0f) * Constants.Pixel);
            var x = _xOffset + (i * segment * Constants.Pixel) + (halfSegment * Constants.Pixel);
            var note = instantiator.NewPartitionNote(new Vector3(x, y, 0.0f) + transform.position);
            ...
            _notes.Add(new Note(spriteRenderer, keybinding, spriteId, opponentRealm));
        }
        _id = -1;
        NextNote();
    }
    ...
    class Note
    {
        ...
        public Note(SpriteRenderer spriteRenderer, KeyBinding keyBinding, int spriteId, Realm realm)
        {
